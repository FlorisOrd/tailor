...
REQUIRED_FILES = ("AGENTS.md", "PRODUCT.md", "ARCHITECTURE.md", "WORKFLOW.md", "QUALITY.md", "SECURITY.md", "INCIDENT_RESPONSE.md", "DECISIONS.md", ".github/PULL_REQUEST_TEMPLATE.md", ".github/GOVERNANCE_ENFORCEMENT.md", ".github/dependabot.yml", ".github/workflows/governance.yml", POLICY_PATH, ".github/governance/gate-record.schema.json", ".github/governance/legacy-evidence-binding.schema.json", ".github/governance/authorization.schema.json", ".github/governance/GATE_RECORDS.md", "scripts/verify_integration.py", "scripts/validate_gate_records.py", "scripts/validate_evidence.py")
...
def validate_policy(policy: dict[str, object], problems: list[str]) -> None:
    ...
    live=evidence.get("live_reconciliation",{}); require(all(live.get(key) is True for key in ("github_pr_comments_required","complete_remote_ref_discovery","origin_queried_at_validation_time","local_ref_subset_not_authoritative","source_disagreement_fails","missing_history_fails","migration_namespace_discovered")), "live evidence reconciliation is weakened", problems)
    legacy=evidence.get("legacy_migration",{});require(all(legacy.get(key) is True for key in ("append_only","provenance_only","never_approval","legacy_comment_remains_required","exact_snapshot_and_hash_required","mixed_schema_normalization_required","backward_successor_lineage_preserved","supersession_does_not_close_findings","unsupported_schema_fails","modern_downgrade_forbidden")) and legacy.get("canonical_ref_prefix")=="refs/governance/evidence-migrations/" and legacy.get("protocol_cutoff")=="2026-08-20T17:30:00Z" and legacy.get("eligible_records")=={"GATE-REVIEW-05-20260820":{"comment_id":5359092776,"schema_versions":[1]}}, "legacy migration controls are weakened", problems)
...
def main() -> int:
    ...
    parsed_schemas={}
    for schema in (".github/governance/gate-record.schema.json", ".github/governance/legacy-evidence-binding.schema.json", ".github/governance/authorization.schema.json"):
    ...
    binding_schema=parsed_schemas.get(".github/governance/legacy-evidence-binding.schema.json",{});require(set(binding_schema.get("required",[]))==BINDING_FIELDS and binding_schema.get("properties",{}).get("record_type",{}).get("const")=="Legacy Evidence Binding" and binding_schema.get("properties",{}).get("provenance_only",{}).get("const") is True and binding_schema.get("properties",{}).get("not_approval",{}).get("const") is True, "legacy binding schema permits approval or does not require the complete binding", problems)
