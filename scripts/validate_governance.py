...
FINDING_FIELDS = {"finding_id", "severity", "summary", "status", "rechecked_by_gate_record_id"}
...
def validate_policy(policy: dict[str, object], problems: list[str]) -> None:
    ...
    evidence = policy.get("evidence", {}); require(set(evidence)=={"formal_gate_types","publisher_must_be_performing_agent","pr_body_is_authoritative","candidate_identity_fields","required_gate_record_fields","required_finding_fields","finding_severities","finding_statuses","supersession","content_addressing"} and set(evidence.get("formal_gate_types", [])) == {"Independent Code Review", "QA", "Security Review", "Release"} and evidence.get("publisher_must_be_performing_agent") is True and evidence.get("pr_body_is_authoritative") is False, "Gate Record authority is weakened", problems)
    ...
    require(set(evidence.get("required_finding_fields",[]))==FINDING_FIELDS and set(evidence.get("finding_severities",[]))=={"BLOCKING","MAJOR","MINOR","SUGGESTION"} and set(evidence.get("finding_statuses",[]))=={"OPEN","CLOSED"}, "finding schema contract is incomplete", problems)
    supersession=evidence.get("supersession",{}); require(all(supersession.get(key) is True for key in ("reciprocal","acyclic","same_gate_type","same_pr","ordered_timestamp","open_serious_findings_survive")), "supersession contract is weakened", problems)
...
def main() -> int:
    ...
    gate_schema=parsed_schemas.get(".github/governance/gate-record.schema.json",{}); finding_schema=gate_schema.get("properties",{}).get("findings",{}).get("items",{})
    require(set(gate_schema.get("required",[]))==RECORD_FIELDS and set(finding_schema.get("required",[]))==FINDING_FIELDS and finding_schema.get("additionalProperties") is False, "Gate Record JSON Schema does not match canonical recursive contract", problems)
