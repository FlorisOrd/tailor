...
class AuthorizationIntegrationTests(unittest.TestCase):
    ...
    def make_gate(cls,gate,index):
        rid=f"GATE-TEST-{index:03d}"; record={"schema_version":1,"gate_record_id":rid,"gate_type":gate,"agent_role":gate,"agent_id":f"agent-{index}","pr_number":1,"base_sha":cls.base,"candidate_sha":cls.candidate,"candidate_tree":cls.tree,"timestamp":f"2026-08-20T1{index}:00:00Z","scope":"factory","checks":["tests"],"findings":[],"disposition":"PASS","repository_state_changed":False,"supersedes":None,"superseded_by":None}
    ...
    def test_unresolved_historical_major_in_complete_ref_ledger_fails(self):
        rid="GATE-HISTORICAL-FAIL"; old={"schema_version":1,"gate_record_id":rid,"gate_type":"Independent Code Review","agent_role":"Independent Code Review","agent_id":"old-reviewer","pr_number":1,"base_sha":self.base,"candidate_sha":self.candidate,"candidate_tree":self.tree,"timestamp":"2026-08-20T10:00:00Z","scope":"old review","checks":["review"],"findings":[{"finding_id":"F-HISTORY","severity":"MAJOR","summary":"open","status":"OPEN","rechecked_by_gate_record_id":None}],"disposition":"FAIL","repository_state_changed":False,"supersedes":None,"superseded_by":None}
        self.git("checkout","-q",self.candidate); self.write(".github/governance/gate-record.json",json.dumps(old)); self.git("add","--all"); self.git("commit","-q","-m","historical fail"); commit=self.sha(); ref=f"refs/governance/gate-records/pr-1/{self.candidate}/{rid}"; self.git("update-ref",ref,commit)
        try: self.fails(self.integration)
        finally: self.git("update-ref","-d",ref)
